...
class TaskHiveRepository implements ITaskRepository {
  ...
  static final box = Hive.openBox<String>(RepositoryHelper.taskBucketName);

  @override
  Future<List<Task>?> getAll() async {
    final tasks = <Task>[];
    final taskList = (await box).values.toList();
  ...
  Future<Task?> getTask(int taskId) async {
    try {
      final taskData = (await box).get(taskId);
  ...
  Future<Task?> saveTask(Task task) async {
    try {
      ...
      await (await box).put(task.id, decodedTask);
      (await box).flush();
      return task;
    } catch (e) {
      return null;
  ...
  Future<Task?> updateTask(int taskId, Task newTask) async {
    ...
    await (await box).putAt(taskId, json.encode(TaskModel.toJson(newTask)));
    await (await box).flush();
